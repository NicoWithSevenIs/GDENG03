...
void Camera::Update(float delta_time, Matrix4x4, Matrix4x4)
{
    if (m_subject)
    {
        ...
        Vector3D camera_pos = Vector3D(x, y, z) + subject_pos;

        m_transform.setTransform(camera_pos);
        m_transform.lookAt(subject_pos);
    }

    Matrix4x4 camera_world = m_transform.GetTransformationMatrix();

    m_view_matrix = camera_world;
    m_view_matrix.inverse();
